...
export default function ShowroomNewReturnPage() {
  ...
  const handleInputChange = async (val: string) => {
    ...
    if (val.trim().length >= 2) {
      ...
      try {
        ...
        const billsPromise = fetch(`/api/showroom/bills?search=${encodeURIComponent(cleanedVal)}`).then(res => res.ok ? res.json() : []);
        const ordersPromise = fetch(`/api/orders?all=true&search=${encodeURIComponent(cleanedVal)}`).then(res => res.ok ? res.json() : ({} as any));
  ...
  const searchBill = async (searchNo?: string) => {
    ...
    try {
      ...
      const billRes = await fetch(`/api/showroom/bills?invoiceNo=${encodeURIComponent(targetNo.trim())}`);
      if (billRes.ok) {
      ...
      // 2. Try online orders
      const orderRes = await fetch(`/api/orders?all=true&search=${encodeURIComponent(targetNo.trim())}`);
      if (orderRes.ok) {
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4 p-6 border rounded-xl bg-slate-50 dark:bg-slate-900/50">
              ...
              <Button 
                ...
                disabled={submitting || !returnItems.some((item: any) => item.returnQty > 0)}
              >
